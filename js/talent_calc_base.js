...
var tc_element=
[
 {frame:'icon-over-grey.gif',   dir:'../../module/acp_module/images/bwicons/', color:'#1AFF1A', arrow:'green'},
 {frame:'icon-over-grey.gif',   dir:'../../module/acp_module/images/bwicons/', color:'#FFFFFF', arrow:'grey'},
 {frame:'icon-over-yellow.gif', dir:'../../module/acp_module/images/icons/',   color:'#FFD100', arrow:'yellow'},
 {frame:'icon-over-green.gif',  dir:'../../module/acp_module/images/icons/',   color:'#1AFF1A', arrow:'green'}
];
...
function tc_updateElment(id, mode)
{
	...
	for(var img = div.firstChild; img!=null; img = img.nextSibling)
	{
		...
		else if (img.id=='brd') img.src = '../../module/acp_module/images/talentcalc/'+m.frame;
		else if (img.id=='rnk'){img.style.color = m.color;img.innerHTML = tc_bild[id] + '/' + d.max;}
		else if (img.id=='arw') img.src = '../../module/acp_module/images/talentcalc/'+d.depend.img+'-'+m.arrow+'.gif';
	}
}
function tc_renderTree(element)
{
  ...
  for (var i=0; i<tc_tabs; i++)
  {
	...
	text+= '<img src="../../module/acp_module/images/talentcalc/t_'+tc_showclass+'_'+i+'.jpg" style="position: absolute; width: '+(58*tc_col+10)+'px; height:'+(58*tc_row+10)+'px;"></div>';
	...
	for (var row = 0; row < tc_row; row++)
	{
	  ...
	  for (var col = 0; col < tc_col; col++)
	  {
		...
		if (d)
		{
			...
			text+= '<img id="brd" src="../../module/acp_module/images/talentcalc/'+m.frame+'" style="position: absolute; left: 0px; top: 0px;">';
			text+= '<div id="rnk" style="position: absolute; left: 34px; top: 38px; color:'+m.color+';">'+ tc_bild[id] + '/' + d.max + '</div>';
			if (d.depend)
				text+= '<img id="arw" src="../../module/acp_module/images/talentcalc/'+d.depend.img+'-'+m.arrow+'.gif" style="position: absolute; border: 0px; left: '+d.depend.x+'px; top: '+d.depend.y+'px;">';
			text+= '</div>';
		}
		text+= '</td>';
	  }
  ...
  for (i = 0;i<tc_tabs;i++)
  {
	text+= '<td><div style="position: relative;">'
		+ '<img align="left" src="../../module/acp_module/images/talentcalc/'+tc_showclass+'_'+i+'.png">'
		+ tc_name[i] + ': '
		+ '<span id="talent_point_'+i+'">'+tc_points[i]+'</span>'
		+ '<a href=# onclick="return tc_resetTree('+i+');" style="position: absolute; top: 2px; right: 2px;"><img src="../../module/acp_module/images/talentcalc/reset.gif" border=0></a>'
		+ '</div></td>';
  }
  text+= '</tr>';
  text+= '</tbody></table>';
